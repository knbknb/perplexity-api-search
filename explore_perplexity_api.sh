...
extract_json_from_html_report() {
    # not robust: xmllint --html --xpath "//code/text()" "$report_file"
    # robust way to extract the JSON from the HTML reports: xidel
    local report="$1"
    local json_outfile="$2"
    xidel --input "$report_file" --html -s -e  "//code/text()" \
    | tail -n +3 \
...
for model in $models; do
    ...
    #echo "$prompt_json" 
    outfile_json="${SLUG}${model}.json"
    outfile_html="${SLUG}${model}.html"
    query_file="${query_dir}/$outfile_json"
    report_file="${html_outdir}/$outfile_html"
    ...
    if [ ! -f "$report_file" ]; then
        echo "Skipping '$report_file', file unavailable"
        continue
    else 
        # extract the JSON from the HTML reports
        extract_json_from_html_report "$report_file" "$json_outfile"
    fi
